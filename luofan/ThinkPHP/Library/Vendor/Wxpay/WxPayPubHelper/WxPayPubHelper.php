...
//			curl_close($ch);
	...
	{
		file_put_contents('./0601.txt',var_export($xml,true));
		...
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		file_put_contents('./0888.txt',var_export($ch,true));
		$data = curl_exec($ch);
		//返回结果
		file_put_contents('./0999.txt',$data);
		if($data){
			curl_close($ch);
			return $data;
		}
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	/**
	 * 	作用：使用证书post请求xml
	 '<xml><out_trade_no><![CDATA[KQMY201703101615479070]]></out_trade_no>
			<out_refund_no><![CDATA[KQMY201703101615479070-001]]></out_refund_no>
			<total_fee>1</total_fee>
			<refund_fee>1</refund_fee>
			<op_user_id>1439474402</op_user_id>
			<appid><![CDATA[wxe9de09472bb0b6a4]]></appid>
			<mch_id>1439474402</mch_id>
			<nonce_str><![CDATA[5p6butyy5av518rl90x46yt3jsjrx4k6]]></nonce_str>
			<sign><![CDATA[95FF57F41D69D014760EC5D18FF293DC]]></sign>
</xml>'
	 */
	function postXmlSSL()
	{	
	    $xml = $this->createXml();
		file_put_contents('./403.txt',var_export($xml,true));
...
class Refund_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
		    ...
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
			file_put_contents('./205.txt',$this->arrayToXml($this->parameters));
		    return  $this->arrayToXml($this->parameters);
		}catch (SDKRuntimeException $e)
		{
			die($e->errorMessage());
	...
	{		
		...
		$this->postXmlSSL();
		file_put_contents('./303.txt',$this->postXmlSSL());
		$this->result = $this->xmlToArray($this->response);
		return $this->result;
	}
