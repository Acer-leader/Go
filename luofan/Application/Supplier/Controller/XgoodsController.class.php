...
class XgoodsController extends CommonController {
        ...
        $m  = M("cate");if(IS_AJAX){
            ...
            foreach($arr as $key => $val){
                ...
                if($data['pid']){
                    ...
                    if($res){
                        $this->ajaxReturn(array("status"=>1,"info"=>" 删除成功","url"=>U('Supplier/Xgoods/cate')));
                    }else{
                        $this->ajaxReturn(array("status"=>0,"info"=>"删除失败"));
                    }
                }else{
    ...
    public function addGoods(){
        ...
        if(IS_POST){
            ...
            if($res){
                ...
                $this->success("新增成功！",U('Supplier/Xgoods/goodslist'));exit;
            }else{
                $this->error("新增失败！");
            }
    ...
    public function editGoods(){
        if(IS_POST){
            ...
            if($res !== false){
                ...
                $this->success("修改成功!",U('Supplier/Xgoods/goodslist'));exit;
            }else{
                $this->error("修改失败！");
            }
        }
