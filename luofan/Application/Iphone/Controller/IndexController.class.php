...
class IndexController extends PublicController {
    ...
    public function index(){
        ...
        $m = M('news');
        //今日头条
        $list1 = $m->order('add_time desc')->limit(0,6)->select();
        //贷款攻略
        $list1_cate  = M("news_cate")->where(array('sort'=>101,"pid"=>0))->find();
        $where['cate_id'] = $list1_cate['id'];
        $list2 = $m->where($where)->order('sort asc')->limit(0,6)->select();

        $this->assign('list1',$list1);
        $this->assign('list2',$list2);
        session('loan',null);
        $ban=M('banner_view');
        $banner=$ban->where(array('type'=>2))->select();
    ...
    public function freeApply(){
        if(IS_POST){
            ...
            //=======================检测验证码=================
            $res = checkMessage($telephone, $code, 4);
            $res['status']  =1;
            //=======================检测验证码=================
            if($res['status']!=1){
                $this->ajaxReturn($res);
    ...
    public function dingwei(){
        ...
        foreach($c as $key=>$val){
            ...
            if($val['name']==$val['shortname']&&$val['leveltype']==1){
                $zcity[]  =  $c[$key]; 
            }else{
                $province[]   =   $c[$key];
            }
