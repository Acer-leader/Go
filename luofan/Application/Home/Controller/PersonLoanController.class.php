...
class PersonLoanController extends LoanController {
    ...
    public function index(){
        ...
        $is_organ = I('get.is_organ'); //机构类型id
        if($is_organ == 1){
            $this->assign('title',"民间借贷-牛轰轰款");
        }
        if($money){
            //查询区间
        ...
        $where['cate_id'] = $this->lontypeArr['single'];
        $where['is_del']  = 0;
        //$where['is_sale'] = 1;
        ...
        $this->selectGuwenLon($lontype,8);  //顾问
        
        // $this->selectArticle($lontype,'gonglue',7);  //文章
        $articlelists=M("newsgonglue")->where(array("cate_id"=>37,"cate_pid"=>93))->select();
        // dump($articlelists);
        // die;
        $this->assign('articlelists',$articlelists);
        
        $this->assign('a_b_pzy',$this->advBanner(5));  //中右上
        $this->assign('a_b_z',$this->advBanner(6,2));  //中
        $this->assign('a_b_xy',$this->advBanner(7));  //下右
        
        $hot = M('Problems')->order('browse desc')->limit('6')->select();//热门话题
        $this->assign('hot',$hot);

        $l_o = M('loan_order');
	$l_o_r = $l_o ->where(array('cate_id'=>1))->order('id desc')->limit(3)->select();
   ...
   //攻略卡
    public function gonglue(){
        ...
        $count = $i->where(array("cate_name"=>"个贷攻略"))->count();
        $Page = getpage($count, 12);
        $show = $Page->show();//分页显示输出
        $res4 = $i->where(array("cate_name"=>"个贷攻略"))->limit($Page->firstRow . ',' . $Page->listRows)->order('sort asc')->select();

        
        //seo设置
        // $seo = M('SeoView')->where(array('controller'=>'personload','function'=>'gonglue'))->find();
        // $title = str_replace('$title','个贷攻略',$seo['title']);
    ...
    public function personDetail(){
        ...
        $res = M('SeoView')->where(array('controller'=>'PersonLoan','function'=>'personDetail'))->find();
        ...
        $hot = M('Problems')->order('browse desc')->order('id desc')->limit('5')->select();//热门话题
        $this->assign('hot',$hot);
        $hot1 = M('Problems')->order('browse desc')->order('id desc')->limit('8')->select();//热门话题
        ...
        //热门产品
        $hot = $this->selectCityLon('single',$identityid='',$houseid='',$carid='',$honourid='',$city='',$limit=7,$is_sale=1,$is_del=0);
    ...
    public function serch(){
        ...
        $lists = $this->selectCityLon('single',$identityid,$houseid,$carid,$honourid,$this->city,$organ,$pledge,$repay,5,$is_organ);  //查询显示产品
        $html = '';
        if($lists[0])
        {
            foreach($lists[1] as $key=>$val){
                ...
                <div class="gedai2_list_left">';
                    ...
                    $html .='<div class="gedai2_list_img">
                        <a href="'.U('/Home/PersonLoan/personDetail',array('id'=>$val['id'])).'">
                            <img src="'.$val['logo_pic1'].'" />
                        </a>
                    </div>
                </div>
                <div class="gedai2_list_right">
                    <div class="gedai2_list_top"> <a href="'.U('/Home/PersonLoan/personDetail',array('id'=>$val['id'])).'">'.$val['title'].'</a>
                    ...
                    <div class="gedai2_list_bottom">
                        ...
                        <div class="gedai2_bottom_right">
                            ...
                            <div class="gedai2_more"> <a href="'.U('/Home/PersonLoan/personDetail',array('id'=>$val['id'])).'">查看详情</a> </div>
                        </div>
                        <div class="clear"></div>
                    </div>
                    </div>
    ...
    public function answers(){
        ...
        $answer = M('Problems')->where($data)->count();//回答量
        ...
        foreach($problem as $key=>$val){
            ...
            if($val['id'] == $id){
                ...
                $data['id'] = $id;
                M('Problems')->save($data);
                $res = $problem[$key];
                $answerContent = M('Answers')->where(array('p_id'=>$id))->select();
                $title = $res['title'];
...
//                $this->assign('res',$res);
        ...
        $hot = M('Problems')->order('browse desc')->limit('5')->select();//热门话题
        $this->assign('hot',$hot);
        //推荐阅读
    ...
    public function addAnswer(){
        if(IS_AJAX){
            ...
            M('Problems')->where(array('id'=>$data['p_id']))->setInc('reply');
            $res = M('Answers')->add($data);
            if($res){
    ...
    public function ajaxauth(){
        $id = $this->user_id;
        if(!empty($id)){
            $data = M('Member')->field('telephone,realname,houseid,carid,month_money')->find($id);
            if(empty($data['houseid']) || empty($data['realname']) || empty($data['carid']) || empty($data['month_money'])){
                $this->ajaxReturn(array('status'=>0));
            }else{
                //车子
                if($data['carid']){
                    $res = M('Cate')->field('classname')->find($data['carid']);
                    $data['carid'] = $res['classname'];
                }
                //房子
                if($data['houseid']){
                    $res1 = M('Cate')->field('classname')->find($data['houseid']);
                    $data['houseid'] = $res1['classname'];
                }
